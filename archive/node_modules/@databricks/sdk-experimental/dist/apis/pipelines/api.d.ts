import { ApiClient } from "@databricks/sdk-experimental/src/api-client";
import * as pipelines from "@databricks/sdk-experimental/src/apis/pipelines/model";
import { EmptyResponse } from "@databricks/sdk-experimental/src/types";
import { ApiError, ApiRetriableError } from "@databricks/sdk-experimental/src/apis/apiError";
import { Context } from "@databricks/sdk-experimental/src/context";
import { Waiter } from "@databricks/sdk-experimental/src/wait";
export declare class PipelinesRetriableError extends ApiRetriableError {
    constructor(method: string, message?: string);
}
export declare class PipelinesError extends ApiError {
    constructor(method: string, message?: string);
}
/**
 * The Lakeflow Spark Declarative Pipelines API allows you to create, edit,
 * delete, start, and view details about pipelines.
 *
 * Spark Declarative Pipelines is a framework for building reliable,
 * maintainable, and testable data processing pipelines. You define the
 * transformations to perform on your data, and Spark Declarative Pipelines
 * manages task orchestration, cluster management, monitoring, data quality, and
 * error handling.
 *
 * Instead of defining your data pipelines using a series of separate Apache
 * Spark tasks, Spark Declarative Pipelines manages how your data is transformed
 * based on a target schema you define for each processing step. You can also
 * enforce data quality with Spark Declarative Pipelines expectations.
 * Expectations allow you to define expected data quality and specify how to
 * handle records that fail those expectations.
 */
export declare class PipelinesService {
    readonly client: ApiClient;
    constructor(client: ApiClient);
    private _create;
    /**
     * Creates a new data processing pipeline based on the requested
     * configuration. If successful, this method returns the ID of the new
     * pipeline.
     */
    create(request: pipelines.CreatePipeline, context?: Context): Promise<pipelines.CreatePipelineResponse>;
    private _delete;
    /**
     * Deletes a pipeline. Deleting a pipeline is a permanent action that stops
     * and removes the pipeline and its tables. You cannot undo this action.
     */
    delete(request: pipelines.DeletePipelineRequest, context?: Context): Promise<EmptyResponse>;
    private _get;
    /**
     * Get a pipeline.
     */
    get(request: pipelines.GetPipelineRequest, context?: Context): Promise<pipelines.GetPipelineResponse>;
    private _getPermissionLevels;
    /**
     * Gets the permission levels that a user can have on an object.
     */
    getPermissionLevels(request: pipelines.GetPipelinePermissionLevelsRequest, context?: Context): Promise<pipelines.GetPipelinePermissionLevelsResponse>;
    private _getPermissions;
    /**
     * Gets the permissions of a pipeline. Pipelines can inherit permissions from
     * their root object.
     */
    getPermissions(request: pipelines.GetPipelinePermissionsRequest, context?: Context): Promise<pipelines.PipelinePermissions>;
    private _getUpdate;
    /**
     * Gets an update from an active pipeline.
     */
    getUpdate(request: pipelines.GetUpdateRequest, context?: Context): Promise<pipelines.GetUpdateResponse>;
    private _listPipelineEvents;
    /**
     * Retrieves events for a pipeline.
     */
    listPipelineEvents(request: pipelines.ListPipelineEventsRequest, context?: Context): AsyncIterable<pipelines.PipelineEvent>;
    private _listPipelines;
    /**
     * Lists pipelines defined in the Spark Declarative Pipelines system.
     */
    listPipelines(request: pipelines.ListPipelinesRequest, context?: Context): AsyncIterable<pipelines.PipelineStateInfo>;
    private _listUpdates;
    /**
     * List updates for an active pipeline.
     */
    listUpdates(request: pipelines.ListUpdatesRequest, context?: Context): Promise<pipelines.ListUpdatesResponse>;
    private _setPermissions;
    /**
     * Sets permissions on an object, replacing existing permissions if they
     * exist. Deletes all direct permissions if none are specified. Objects can
     * inherit permissions from their root object.
     */
    setPermissions(request: pipelines.PipelinePermissionsRequest, context?: Context): Promise<pipelines.PipelinePermissions>;
    private _startUpdate;
    /**
     * Starts a new update for the pipeline. If there is already an active update
     * for the pipeline, the request will fail and the active update will remain
     * running.
     */
    startUpdate(request: pipelines.StartUpdate, context?: Context): Promise<pipelines.StartUpdateResponse>;
    private _stop;
    /**
     * Stops the pipeline by canceling the active update. If there is no active
     * update for the pipeline, this request is a no-op.
     */
    stop(stopRequest: pipelines.StopRequest, context?: Context): Promise<Waiter<EmptyResponse, pipelines.GetPipelineResponse>>;
    private _update;
    /**
     * Updates a pipeline with the supplied configuration.
     */
    update(request: pipelines.EditPipeline, context?: Context): Promise<EmptyResponse>;
    private _updatePermissions;
    /**
     * Updates the permissions on a pipeline. Pipelines can inherit permissions
     * from their root object.
     */
    updatePermissions(request: pipelines.PipelinePermissionsRequest, context?: Context): Promise<pipelines.PipelinePermissions>;
}
//# sourceMappingURL=api.d.ts.map