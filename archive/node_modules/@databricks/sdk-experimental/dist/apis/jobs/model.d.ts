import * as compute from "@databricks/sdk-experimental/src/apis/compute";
export type AuthenticationMethod = "OAUTH" | "PAT";
export interface BaseJob {
     ...
     * that are not shown. They can be accessed via :method:jobs/get endpoint. It
     * is only relevant for API 2.2 :method:jobs/list requests with
     * `expand_tasks=true`.
     */
    has_more?: boolean;
    /**
     ...
     */
    trigger_state?: TriggerStateProto;
}
export interface BaseRun {
     ...
     */
    effective_performance_target?: PerformanceTarget;
    /**
     ...
     * It is only relevant for API 2.2 :method:jobs/listruns requests with
     * `expand_tasks=true`.
     */
    has_more?: boolean;
    /**
     ...
     * runs the field is populated with the job run ID. For task runs, the field
     * is populated with the ID of the job run that the task run belongs to.
     */
    job_run_id?: number;
    /**
     ...
     * arrival. * `CONTINUOUS`: Indicates a run that is triggered by a continuous
     * job. * `TABLE`: Indicates a run that is triggered by a table update. *
     * `CONTINUOUS_RESTART`: Indicates a run created by user to manually restart
     * a continuous job run. * `MODEL`: Indicates a run that is triggered by a
     * model update.
     */
    trigger?: TriggerType;
    /**
 ...
 */
export type CleanRoomTaskRunLifeCycleState = "BLOCKED" | "INTERNAL_ERROR" | "PENDING" | "QUEUED" | "RUNNING" | "RUN_LIFE_CYCLE_STATE_UNSPECIFIED" | "SKIPPED" | "TERMINATED" | "TERMINATING" | "WAITING_FOR_RETRY";
/**
 * Copied from elastic-spark-common/api/messages/runs.proto. Using the original
 * definition to avoid cyclic dependency.
 */
export type CleanRoomTaskRunResultState = "CANCELED" | "DISABLED" | "EVICTED" | "EXCLUDED" | "FAILED" | "MAXIMUM_CONCURRENT_RUNS_REACHED" | "RUN_RESULT_STATE_UNSPECIFIED" | "SUCCESS" | "SUCCESS_WITH_FAILURES" | "TIMEDOUT" | "UPSTREAM_CANCELED" | "UPSTREAM_EVICTED" | "UPSTREAM_FAILED";
/**
...
export interface CleanRoomTaskRunState {
     ...
     * future releases.
     */
    life_cycle_state?: CleanRoomTaskRunLifeCycleState;
    /**
     ...
     * future releases.
     */
    result_state?: CleanRoomTaskRunResultState;
}
/**
...
export interface CleanRoomsNotebookTaskCleanRoomsNotebookTaskOutput {
    /**
     * The run state of the clean rooms notebook task.
     */
    clean_room_job_run_state?: CleanRoomTaskRunState;
    /**
     ...
     */
    output_schema_info?: OutputSchemaInfo;
}
...
export interface ClusterSpec {
     ...
     * jobs and tasks on new clusters for greater reliability
     */
     ...
     */
    libraries?: Array<compute.Library>;
    /**
...
export type Condition = "ALL_UPDATED" | "ANY_UPDATED";
 ...
 */
export type ConditionTaskOp = "EQUAL_TO" | "GREATER_THAN" | "GREATER_THAN_OR_EQUAL" | "LESS_THAN" | "LESS_THAN_OR_EQUAL" | "NOT_EQUAL";
export interface Continuous {
     ...
     * Defaults to NEVER.
     */
    task_retry_mode?: TaskRetryMode;
}
export interface CreateJob {
    /**
     * List of permissions to set on the job.
     */
    access_control_list?: Array<JobAccessControlRequest>;
    /**
     ...
     * by serverless tasks of this job. For serverless notebook tasks, if the
     ...
     * environment. For other serverless tasks, the task environment is required
     * to be specified using environment_key in the task settings.
     */
    environments?: Array<JobEnvironment>;
    /**
     ...
     * endpoints return only 100 tasks. If more than 100 tasks are available, you
     * can paginate through them using :method:jobs/get. Use the
...
export interface DashboardPageSnapshot {
    ...
    widget_error_details?: Array<WidgetErrorDetail>;
}
/**
...
export interface DashboardTaskOutput {
     ...
     */
    page_snapshots?: Array<DashboardPageSnapshot>;
}
/**
...
export interface DbtCloudTaskOutput {
     ...
     */
    dbt_cloud_job_run_id?: number;
    /**
     * Steps of the job run as received from dbt Cloud
     */
    dbt_cloud_job_run_output?: Array<DbtCloudJobRunStep>;
    /**
     * Url where full run details can be viewed
     */
    dbt_cloud_job_run_url?: string;
}
 ...
 * output
 */
export type DbtPlatformRunStatus = "CANCELLED" | "ERROR" | "QUEUED" | "RUNNING" | "STARTING" | "SUCCESS";
...
export interface DbtPlatformTaskOutput {
     ...
     */
    dbt_platform_job_run_id?: string;
    /**
     * Steps of the job run as received from dbt platform
     */
    dbt_platform_job_run_output?: Array<DbtPlatformJobRunStep>;
    /**
     * Url where full run details can be viewed
     */
    dbt_platform_job_run_url?: string;
    /**
 ...
 */
export interface EnforcePolicyComplianceForJobResponseJobClusterSettingsChange {
    /**
     ...
     * number, a boolean, or a string) converted to a string. This is intended to
     * be read by a human. The typed new value of this field can be retrieved by
...
export interface EnforcePolicyComplianceRequest {
    /**
...
export interface EnforcePolicyComplianceResponse {
    /**
     * Whether any changes have been made to the job cluster settings for the job
     * to become compliant with its policies.
     */
    has_changes?: boolean;
    /**
     ...
     * policies.
     */
    job_cluster_changes?: Array<EnforcePolicyComplianceForJobResponseJobClusterSettingsChange>;
    /**
...
export interface ForEachStats {
     ...
     */
    error_message_stats?: Array<ForEachTaskErrorMessageStats>;
    /**
     * Describes stats of the iteration. Only latest retries are considered.
     */
    task_run_stats?: ForEachTaskTaskRunStats;
}
...
export interface ForEachTaskErrorMessageStats {
    /**
...
export interface ForEachTaskTaskRunStats {
    /**
     * Describes the iteration runs having an active lifecycle state or an active
     * run sub state.
     */
...
export interface GenAiComputeTask {
     ...
     * the script will be retrieved from the local Databricks workspace. When set
     * to `GIT`, the script will be retrieved from a Git repository defined in
...
export interface GetJobPermissionLevelsRequest {
    /**
     * The job for which to get or manage permissions.
     */
    job_id: string;
}
export interface GetJobPermissionLevelsResponse {
    /**
     * Specific permission levels
     */
    permission_levels?: Array<JobPermissionsDescription>;
}
export interface GetJobPermissionsRequest {
    /**
     * The job for which to get or manage permissions.
     */
    job_id: string;
...
export interface GetPolicyComplianceRequest {
    /**
     * The ID of the job whose compliance status you are requesting.
     */
    job_id: number;
}
export interface GetPolicyComplianceResponse {
    /**
...
export type GitProvider = "awsCodeCommit" | "azureDevOpsServices" | "bitbucketCloud" | "bitbucketServer" | "gitHub" | "gitHubEnterprise" | "gitLab" | "gitLabEnterpriseEdition";
/**
...
export interface JobAccessControlRequest {
    /**
     * name of the group
     */
    group_name?: string;
...
export interface JobAccessControlResponse {
    /**
     * All permissions.
     */
    all_permissions?: Array<JobPermission>;
    /**
...
export interface JobEmailNotifications {
     ...
     */
    on_duration_warning_threshold_exceeded?: Array<string>;
    /**
     ...
     * the `health` field using the following metrics: `STREAMING_BACKLOG_BYTES`,
     ...
     * minutes.
     */
    on_streaming_backlog_exceeded?: Array<string>;
    /**
 ...
 * Permission level
 */
export type JobPermissionLevel = "CAN_MANAGE" | "CAN_MANAGE_RUN" | "CAN_VIEW" | "IS_OWNER";
export interface JobPermissions {
    access_control_list?: Array<JobAccessControlResponse>;
    object_id?: string;
    object_type?: string;
}
export interface JobPermissionsDescription {
    description?: string;
    /**
     * Permission level
     */
...
export interface JobPermissionsRequest {
    access_control_list?: Array<JobAccessControlRequest>;
    /**
     * The job for which to get or manage permissions.
     */
 ...
 *
 * Either `user_name` or `service_principal_name` should be specified. If not, an
 * error is thrown.
 */
 ...
 * job specification. Import the remote job specification from UI to make the job
 * fully synced. * `DISCONNECTED`: The job is temporary disconnected from the
 * remote job specification and is allowed for live edit. Import the remote job
 ...
 * `STREAMING_BACKLOG_BYTES`: An estimate of the maximum bytes of data waiting to
 * be consumed across all streams. This metric is in Public Preview. *
 ...
 * Public Preview. * `STREAMING_BACKLOG_FILES`: An estimate of the maximum number
 * of outstanding files across all streams. This metric is in Public Preview.
 */
 ...
 * specified threshold.
 */
export type JobsHealthOperator = "GREATER_THAN";
...
export interface JobsHealthRules {
    rules?: Array<JobsHealthRule>;
}
export interface ListJobComplianceForPolicyResponse {
    /**
     * A list of jobs and their policy compliance statuses.
     */
    jobs?: Array<JobCompliance>;
    /**
...
export interface ListJobComplianceRequest {
    /**
     * Use this field to specify the maximum number of results to be returned by
     * the server. The server may further constrain the maximum number of results
...
export interface ListRunsRequest {
     ...
     * than 25. The default value is 20. If a request specifies a limit of 0, the
...
export interface NotebookTask {
     ...
     * initiated by a call to :method:jobs/run Now with parameters specified, the
     ...
     * `git_source`. If the value is empty, the task will use `GIT` if
     * `git_source` is defined and `WORKSPACE` otherwise. * `WORKSPACE`: Notebook
     * is located in Databricks workspace. * `GIT`: Notebook is located in cloud
     * Git provider.
     */
    source?: Source;
    /**
...
/**
 * PerformanceTarget defines how performant (lower latency) or cost efficient the
 * execution of run on serverless compute should be. The performance mode on the
 * job or pipeline should map to a performance setting that is passed to Cluster
 * Manager (see cluster-common PerformanceTarget).
 */
export type PerformanceTarget = "PERFORMANCE_OPTIMIZED" | "STANDARD";
...
export type PeriodicTriggerConfigurationTimeUnit = "DAYS" | "HOURS" | "WEEKS";
 ...
 */
export type QueueDetailsCodeCode = 
/**
...
export interface RepairRun {
     ...
     * run. This field overrides the performance target defined on the job level.
     *
     * * `STANDARD`: Enables cost-efficient execution of serverless workloads. *
     * `PERFORMANCE_OPTIMIZED`: Prioritizes fast startup and execution times
     * through rapid scaling and optimized cluster performance.
...
export interface Run {
    ...
    has_more?: boolean;
    /**
     * Only populated by for-each iterations. The parent for-each task is located
     * in tasks array.
     */
    iterations?: Array<RunTask>;
    /**
     * A list of job cluster specifications that can be shared and reused by
     * tasks of this job. Libraries cannot be declared in a shared job cluster.
     * You must declare dependent libraries in task settings. If more than 100
...
export interface RunForEachTask {
    /**
     * An optional maximum allowed number of concurrent runs of the task. Set
     * this value if you want to be able to execute multiple runs of the task
     * concurrently.
 ...
 * period. If a run on a new cluster ends in the `INTERNAL_ERROR` state, the Jobs
 * service terminates the cluster as soon as possible. This state is terminal. *
 ...
 */
export type RunLifecycleStateV2State = "BLOCKED" | "PENDING" | "QUEUED" | "RUNNING" | "TERMINATED" | "TERMINATING" | "WAITING";
...
export interface RunOutput {
     ...
     */
    clean_rooms_notebook_output?: CleanRoomsNotebookTaskCleanRoomsNotebookTaskOutput;
    /**
     * The output of a dashboard task, if available
     */
    dashboard_output?: DashboardTaskOutput;
    /**
     * Deprecated in favor of the new dbt_platform_output
     */
    dbt_cloud_output?: DbtCloudTaskOutput;
    /**
     * The output of a dbt task, if available.
     */
    dbt_output?: DbtOutput;
    dbt_platform_output?: DbtPlatformTaskOutput;
    /**
     ...
     * to return the first 5 MB of the output. To return a larger result, use the
     * [ClusterLogConf] field to configure log storage for the job cluster.
     *
...
/**
 * A value indicating the run's result. The possible values are: * `SUCCESS`: The
 * task completed successfully. * `FAILED`: The task completed with an error. *
 * `TIMEDOUT`: The run was stopped after reaching the timeout. * `CANCELED`: The
 * run was canceled at user request. * `MAXIMUM_CONCURRENT_RUNS_REACHED`: The run
 * was skipped because the maximum concurrent runs were reached. * `EXCLUDED`:
 ...
 * failures; leaf tasks were successful. * `UPSTREAM_FAILED`: The run was skipped
 * because of an upstream failure. * `UPSTREAM_CANCELED`: The run was skipped
 * because an upstream task was canceled. * `DISABLED`: The run was skipped
 * because it was disabled explicitly by the user.
 */
...
export interface RunTask {
     ...
     * task does not require a cluster to execute and does not support retries or
     * notifications.
     */
...
export interface SparkJarTask {
     ...
     * `libraries` field instead. For serverless compute, provide a `jar` though
     * the `java_dependencies` field inside the `environments` list.
     *
...
export interface SqlTaskSubscription {
     ...
     * mutually exclusive with destination_id. You cannot set both destination_id
     * and user_name for subscription notifications.
     */
    user_name?: string;
}
export type StorageMode = "DIRECT_QUERY" | "DUAL" | "IMPORT";
...
export interface Subscription {
     ...
     */
    subscribers?: Array<SubscriptionSubscriber>;
}
...
export interface TableTriggerState {
    last_seen_table_states?: Array<TableState>;
    /**
     * Indicates whether the trigger is using scalable monitoring.
     */
    using_scalable_monitoring?: boolean;
}
export interface TableUpdateTriggerConfiguration {
     ...
     * for the specified time and can be used to wait for a series of table
     * updates before triggering a run. The minimum allowed value is 60 seconds.
     */
    wait_after_last_change_seconds?: number;
}
export interface Task {
     ...
     * task. The task will run only if the `run_if` condition is true. The key is
 ...
 * started.
 */
export type TaskRetryMode = "NEVER" | "ON_FAILURE";
/**
 ...
 * canceled during execution by a user. * `CANCELED`: The run was canceled during
 * execution by the Databricks platform; for example, if the maximum run duration
 * was exceeded. * `SKIPPED`: Run was never executed, for example, if the
 * upstream task run failed, the dependency type condition was not met, or there
 * were no material tasks to execute. * `INTERNAL_ERROR`: The run encountered an
 ...
 * causes might include, but are not limited to: The provided library is invalid,
 ...
 * concurrent runs set for the job. * `MAX_SPARK_CONTEXTS_EXCEEDED`: The run is
 * scheduled on a cluster that has already reached the maximum number of contexts
 * it is configured to create. See: [Link]. * `RESOURCE_NOT_FOUND`: A resource
 * necessary for run execution does not exist. Refer to the state message for
 * further details. * `INVALID_RUN_CONFIGURATION`: The run failed due to an
 * invalid configuration. Refer to the state message for further details. *
 * `CLOUD_FAILURE`: The run failed due to a cloud provider issue. Refer to the
 * state message for further details. * `MAX_JOB_QUEUE_SIZE_EXCEEDED`: The run
 * was skipped due to reaching the job level queue size limit. * `DISABLED`: The
 * run was never executed because it was disabled explicitly by the user. *
 * `BREAKING_CHANGE`: Run failed because of an intentional breaking change in
 * Spark, but it will be retried with a mitigation config.
 *
 * [Link]: https://kb.databricks.com/en_US/notebooks/too-many-execution-contexts-are-open-right-now
 */
export type TerminationCodeCode = "BUDGET_POLICY_LIMIT_EXCEEDED"
/**
...
/**
 * The run failed due to an invalid configuration. Refer to the state message for
 * further details.
 */
 | "INVALID_RUN_CONFIGURATION"
/**
 * The run failed while installing the user-requested library. Refer to the state
 * message for further details. The causes might include, but are not limited to:
 * The provided library is invalid, there are insufficient permissions to install
 ...
 */
 | "RESOURCE_NOT_FOUND"
/**
 * The run was completed with task failures. For more details, refer to the state
 * message or run output.
 */
 | "RUN_EXECUTION_ERROR"
/**
 ...
 */
 | "SKIPPED"
/**
 * The run failed due to an error when accessing the customer blob storage. Refer
 * to the state message for further details.
 */
...
export interface TerminationDetails {
     ...
     * an error while communicating with the Spark Driver. * `CLUSTER_ERROR`: The
     * run failed due to a cluster error. Refer to the state message for further
     * details. * `REPOSITORY_CHECKOUT_FAILED`: Failed to complete the checkout
     * due to an error when communicating with the third party service. *
     ...
     * See: [Link]. * `RESOURCE_NOT_FOUND`: A resource necessary for run
     ...
     * by the user. * `BREAKING_CHANGE`: Run failed because of an intentional
     * breaking change in Spark, but it will be retried with a mitigation config.
     *
     * [Link]: https://kb.databricks.com/en_US/notebooks/too-many-execution-contexts-are-open-right-now
     */
...
/**
 * * `SUCCESS`: The run terminated without any issues * `INTERNAL_ERROR`: An
 * error occurred in the Databricks platform. Please look at the [status page] or
 * contact support if the issue persists. * `CLIENT_ERROR`: The run was
 ...
 * [status page]: https://status.databricks.com/
 */
export type TerminationTypeType = 
/**
...
export interface TriggerStateProto {
    file_arrival?: FileArrivalTriggerState;
...
export interface WebhookNotifications {
     ...
     * run exceeds the threshold specified for the `RUN_DURATION_SECONDS` metric
     * in the `health` field. A maximum of 3 destinations can be specified for
     * the `on_duration_warning_threshold_exceeded` property.
     */
    on_duration_warning_threshold_exceeded?: Array<Webhook>;
    /**
     ...
     * notifications are resent every 30 minutes. A maximum of 3 destinations can
     * be specified for the `on_streaming_backlog_exceeded` property.
     */
    on_streaming_backlog_exceeded?: Array<Webhook>;
    /**
